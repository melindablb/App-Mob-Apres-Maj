...
export default function ChangePassword(){
  ...
  const handleSavePassword = async () => {
    ...
    let email2 = '';
    try {
        const jsonValue = await AsyncStorage.getItem('pwdtemp');
        if (jsonValue !== null) {
          const data = JSON.parse(jsonValue);
          console.log('role', data.roleALT);
          role2 = data.roleALT;
          console.log('mail', data.emailALT);
          email2 = data.emailALT;
        }
        else{
        Alert.alert("Error","An error has occured. Please try again.");
        return;
        }
        AsyncStorage.removeItem('pwdtemp');
    setLoading(true);
    ...
    console.log('Erreur lors de la récupération :', e);
    Alert.alert("Error","An error has occured. Please try again.");
    return;
  }
  };
