...
import { ResetNumTel, ResetNumTelCode } from "@/services/changenumtel"
 ...
 const { user, UpdateData } = useAuth();
  ...
  const handleRequestReset = async (): Promise<void> => {
    ...
    try {
      ...
      const response = await ResetNumTel(formData);
  ...
  const handleVerifyOTP = async (): Promise<void> => {
    ...
    try {
      ...
      const response = await ResetNumTelCode(formData);
    
      if(response.status===200){
         router.replace("./changephonenumb");
      }
      else{
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.container}>
      ...
      <View style={styles.content}>
        <Text style={styles.title2}>Reset Phone Number</Text>
